...
def validate_tds_totals(tds_masters_data, challan_data_list):
    ...
    try:
        ...
        for challan in challan_data_list:
            ...
            if nop:
                challan_totals[nop] = challan_totals.get(nop, 0) + tax_amount
        validation_data = []
...
def update_output_file(template_path, output_path, updated_masters_data, challan_data_list, log_expander):
    ...
    for idx, challan in enumerate(challan_data_list, start=4):
        ...
        surcharge = Decimal(challan.get('surcharge', 0)).quantize(Decimal('1.'), rounding=ROUND_HALF_UP)
        cess = Decimal(challan.get('cess', 0)).quantize(Decimal('1.'), rounding=ROUND_HALF_UP)
        interest = Decimal(challan.get('interest', 0)).quantize(Decimal('1.'), rounding=ROUND_HALF_UP)
        penalty = Decimal(challan.get('penalty', 0)).quantize(Decimal('1.'), rounding=ROUND_HALF_UP)
        ...
        ws_challan.cell(row=row_idx, column=4).value = int(surcharge)
        ws_challan.cell(row=row_idx, column=5).value = int(cess)
        ws_challan.cell(row=row_idx, column=6).value = int(interest)
        ws_challan.cell(row=row_idx, column=7).value = int(penalty)
    ...
    # Build challan_map by nop, but check for multiples
    from collections import defaultdict
    challan_groups = defaultdict(list)
    for c in challan_data_list:
        nop = c.get('nature_of_payment', '').replace(' ', '')
        challan_groups[nop].append(c)
    for nop, group in challan_groups.items():
        if len(group) > 1:
            st.warning(f"Multiple challans found for nature of payment {nop}. Using the first one for deductee mapping.")
    challan_map = {nop: group[0] for nop, group in challan_groups.items() if group}
